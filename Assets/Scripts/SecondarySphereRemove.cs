...
public class SecondarySphereRemove : MonoBehaviour {
    ...
    void Update() {
        ...
        if(OVRInput.Get(OVRInput.Button.SecondaryThumbstickUp)) scale *= 1.2f;
        if(OVRInput.Get(OVRInput.Button.SecondaryThumbstickDown)) scale /= 1.2f;
...
//         actualVoxelPosition = Vector3Int.FloorToInt(parent.transform.rotation*actualVoxelPosition);
