...
public class VoxelControl : MonoBehaviour {
    ...
    public void generateMesh() {
		...
		addVerticies.Clear();
		addTriangles.Clear();
        render.material.color = new Color(0.9725F, 0.3647F, 0.2039F);
    }
    ...
    public (string, int) toObj(string name, int indexOffset) {
        ...
        string value = "o " + name;
        for (int i = 0; i < addVerticies.Count; i++) value +="\nv " + (addVerticies[i].x + chunkX * voxelSize).ToString() + " " + (addVerticies[i].y + chunkY * voxelSize).ToString() + " " + (addVerticies[i].z + chunkZ * voxelSize).ToString();
