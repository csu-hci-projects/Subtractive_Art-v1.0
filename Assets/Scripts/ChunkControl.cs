...
public class ChunkControl : MonoBehaviour {	
    ...
    void Start() {
        filename = "export0";
        print(System.IO.File.Exists(filename + ".obj"));
        print(System.IO.File.Exists(filename));
        int i = 1;
        while(System.IO.File.Exists("C:\\Users\\Abby\\Subtractive_Art-v1.0\\Models\\"+filename+".obj")) filename = filename.Substring(0, filename.Length-1) + i++;
		...
		for (int x = 0; x < xChunks; x++) {
			for (int y = 0; y < yChunks; y++) {
				for (int z = 0; z < zChunks; z++) {
					GameObject newChunk = Instantiate (chunk, new Vector3 (x * chunkSize * voxelSize, y * chunkSize * voxelSize, z * chunkSize * voxelSize) + transform.position, new Quaternion (0, 0, 0, 0), parent.transform) as GameObject;
    ...
    void toFile() {
        ...
        System.IO.File.WriteAllText("C:\\Users\\Abby\\Subtractive_Art-v1.0\\Models\\"+filename+".obj", output);
        Debug.Log("Done!");
    }
...
//             if(g.GetComponent<Renderer>()!= null) g.GetComponent<Renderer>().enabled = false;
//         }
        if(save) toFile();
